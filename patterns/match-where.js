 ...
 * @returns {undefined}
 */
Patterns.checkLegacyMatchWhere
  = function checkLegacyMatchWhere(value, pattern) {
    if (!warnedAboutMatchWhere) {
      console.warn(
        ...
        + 'Prefer using korrigans:k-pattern'
        + ' http://github.com/Korrigans/k-pattern'
      );
    ...
    if (!_.isFunction(testFunc)) {
      throw new TypeError(
        `Expected Match.Where validator to be a function, got ${
          beautifyValue(testFunc)
        }`
      );
    }
    ...
    if (!testResult) {
      throw buildCheckError(
        testResult,
        `Match.Where validator ${
          beautifyPattern(testFunc)
        } to return truthy value`
      );
    }
  };
